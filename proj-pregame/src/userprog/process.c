...
//初始化型号量
  ...
  //获取flie_name，也就是命令行参数的第一个，就是threadname
  char* cmd_line=file_name;
  char *saveptr;
  char *token = strtok_r(cmd_line, " ", &saveptr);
  //将name和命令行都传入startprocess
  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create(token, PRI_DEFAULT, start_process, fn_copy);
  //创建子进程状态
  child_status_t *cs=malloc(sizeof(child_status_t));
  cs->pid=tid;
  cs->exited=false;
  cs->waited=false;
...
static void start_process(void* file_name_) {
  ...
  if (success) {
    ...
    char *arg_addresses[128];  // 保存参数字符串在栈上的地址

    // Step 1: 从后往前把参数字符串压到栈里
    for (int i = argc - 1; i >= 0; i--) {
      size_t len = strlen(argv[i]) + 1;
      sp -= len;
      memcpy(sp, argv[i], len);
      arg_addresses[i] = (char *)sp;
    }

    // Step 2: 栈地址对齐到 8 字节
    while ((uintptr_t)sp % 8 != 0) {
      sp--;
      *sp = 0;
    }
...
int process_wait(pid_t child_pid UNUSED) {
  ...
  child_status_t* child = find_child(cur,child_pid);
  if(child==NULL)
  {
    return -1;
  }
